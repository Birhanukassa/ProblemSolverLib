const readline = require("readline-sync"),
    PLAYER_WINNING_GOAL = 21,
    DEALER_WINNING_GOAL = 17,
    gameScore = {"player": 0,
        "dealer": 0},
    WINNING_SCORE = 5;
...
const displayWelcomeTo21 = () => {
    ...
    console.log();
    console.log("               Rules of the Game Play\n               ----------------------\nThe Game Starts with a standard 52-card deck consisting of the four suits.");
    console.log();
...
function joinWithAnd (cards, joinWith = " and ") {
    ...
    const copyOfCards = [...cards],
        isLastCard = copyOfCards.splice(-1);
    switch (cards.length) {
...
function displayGivenCards (player, dealer, value) {
    ...
    const hasHidenCard = hideDealerCard(dealer);
...
function playerTurn (deck, player, playerValue) {
    ...
    while (true) {
        ...
        const input = validatingStayOrHit();
        if (input === "s") {
...
function dealerTurn (deck, dealer, dealerValue) {
    ...
    while (true) {
        ...
        if (dealerValue < DEALER_WINNING_GOAL) {
            ...
            const updatedDealerValue = sumOfCards(dealer);
...
function isWinner (playerValue, dealerValue) {
    ...
    if (isBusted(playerValue)) {
        ...
        return "Dealer Wins! You are BUSTED!";
...
function displayFinalResults (player, playerValue, dealer, dealerValue) {
    ...
    const finalResult = isWinner(
        playerValue,
        dealerValue
    );
    if (finalResult.trimLeft()[0] === "D") {

        gameScore.dealer += 1;

    }
    if (finalResult.trimLeft()[0] === "Y") {
